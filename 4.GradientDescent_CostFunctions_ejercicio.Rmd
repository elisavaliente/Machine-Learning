...
----------------
```{r}
library(caret) # we need this library to use the function confusion matrix
```
...
pred <- rep(0, length(Y)) # vector to keep the predictions

for (i in 1:length(Y)) { 
  student <- c(1, X[[i,2]], X[[i,3]]) 
  prob_pass <- Sigmoid(t(student) %*% parameters)         
  # probability of passing. 
  if (prob_pass > 0.5) { 
  # students pass with a 5 points grade, that's why   we put probability of a new   student passes is over 0.5 
    pred[i] <- 1
  }
}
...
library(caret)
confusionMatrix(data = as.factor(pred) , reference = as.factor(Y))
```
2. Obtain a graph representing how the cost function evolves depending of the number of iterations. 
```{r}
num_iterac <- 1:1000
costs <- rep(0, length(num_iterac)) # vector in which we keep the cost function

test_cost <- function(X, Y) {
  for (iterations in num_iterac) {
    # Initialize (b, W)
    parameters <- rep(0, ncol(X))
    ...
    # Check evolution
    costs[iterations] <- convergence <- c(CostFunction(parameters, X, Y))
  }
  return(costs) 
}
costs_1 <- test_cost(X,Y)
plot(iteraciones, costs_1, "l", main = "Cost function evolution with changes in the number of iterations", xlab = "Iterations", ylab = "Cost function")
```
